...
function transactionDelta(transaction: { amount: number; type: string; accountId: number; toAccountId: number | null }, accountId: number) {
  ...
  if (transaction.type === "Allocate") return 0;
...
export default async function AccountDetailPage({ params }: AccountDetailPageProps) {
  ...
  const reservedBalance = account.transactions.reduce((total, transaction) => total + ((transaction.type === "Transfer" || transaction.type === "Allocate") && transaction.goalId ? transaction.amount : 0), 0);
